...
def download_chara_icon(id_, star):
    ...
    save_path = ResImg(f'card/icon_unit_{id_}{star}1.png').path
...
class Chara:
    ...
    def icon(self):
        ...
        res = ResImg(f'card/icon_unit_{self.id}{star}1.png')
        if not res.exist:
            res = ResImg(f'card/icon_unit_{self.id}31.png')
        if not res.exist:
            res = ResImg(f'card/icon_unit_{self.id}11.png')
        ...
        if not res.exist:
            res = ResImg(f'card/icon_unit_{self.id}11.png')
        if not res.exist:
            res = ResImg(f'card/icon_unit_{UNKNOWN}31.png')
        return res
...
class ResObj:
    ...
    def url(self):
        ...
        return urljoin(self.res_dir, pathname2url(self.__path))
